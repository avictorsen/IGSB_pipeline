	...
	case \$format in
		\'application\/gzip\'\*\)
		        ...
		        if [ \"\$check\" -gt 100 ];then
                		echo "\nconverting gzipped file to Phred33";
				zcat \$file | awk \'NR \% 4 \=\= 0\' | head;
