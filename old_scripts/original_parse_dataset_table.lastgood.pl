...
for file in ./$fn; do
	...
	check1=`grep "a" \$file | wc -l`;
	check2=`grep "e" \$file | wc -l`;
	if [ \"\$check1\" -gt 2 ] && [ \"\$check2\" -gt 2 ];then
		echo "converting to Phred33";
		perl ./idrpipeline/Phred64_to_Phred33.pl \$file > \$file.temp;
		mv \$file.temp \$file || { exit ;};
	fi;
        echo "\\n\\n";
	cat \$file >> $name.$Config{'fastq_suffix'} || { exit ;};
	rm \$file;
done\n\n};
...
foreach my $d (sort {$a cmp $b} keys %Dataset) {
	...
	foreach my $r (sort {$a <=> $b} keys %{$Dataset{$d}{'IP'}}) {
		...
		print OUT2 "## dataset $d IP replicate $r
			...
			rm -f $ip.$fastq_suffix $ip.sai $ip.sam
			";
	...
	foreach my $r (sort {$a <=> $b} keys %{$Dataset{$d}{'Input'}}) {
		...
		print OUT2 "## dataset $d Input replicate $r
			...
			$samtoolspath/samtools view -Sbh $input\_q30.sam > $input\_q30.bam
                        ...
                        rm -f $input.$fastq_suffix $input.sai $input.sam
			";
...
print OUT2 "mail -s 'Run on '\$HOSTNAME' has finished!' -aFROM:\\<avictorsen\@IGSB.opensciencedatacloud.org\\> avictorsen\@gmail.com < cat $Config{'sp'}_idr.log";
