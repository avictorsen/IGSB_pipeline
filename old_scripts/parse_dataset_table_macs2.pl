...
#			Rscript $Config{'bin_dir'}/run_spp.R -c=$IPs[$i].pr2.tagAlign.gz -i=$d\_Input_Rep0.tagAlign.gz -npeak=$npeaks -x=-500:85 -odir=./ -p=$node -filtchr='.*_[CD].*' -savr -savp -rf -out=$Config{'date'}.cc
		# macs2 peak calls
			...
			macs2 callpeak -t $IPs[$i].tagAlign.gz -c $d\_Input_Rep0.tagAlign.gz -g dm --to-large --nomodel --extsize \$ss -p 0.1 -n $IPs[$i].tagAlign_VS_$d\_Input_Rep0.tagAlign &
			ss=\$(cat $Config{'date'}.cc | grep $IPs[$i].pr1.tagAlign.gz | cut -f3 | head -n 1 | cut -d \",\" -f1)
			macs2 callpeak -t $IPs[$i].pr1.tagAlign.gz -c $d\_Input_Rep0.tagAlign.gz -g dm --to-large --nomodel --extsize \$ss -p 0.1 -n $IPs[$i].pr1.tagAlign_VS_$d\_Input_Rep0.tagAlign &
			ss=\$(cat $Config{'date'}.cc | grep $IPs[$i].pr2.tagAlign.gz | cut -f3 | head -n 1 | cut -d \",\" -f1)
			macs2 callpeak -t $IPs[$i].pr2.tagAlign.gz -c $d\_Input_Rep0.tagAlign.gz -g dm --to-large --nomodel --extsize \$ss -p 0.1 -n $IPs[$i].pr2.tagAlign_VS_$d\_Input_Rep0.tagAlign &\`
			wait \$!
	";
	}
...
#			Rscript $bin_dir/spp_wig_tagAlign.R  --args  --baseDir=./ --outDir=./ --factor=\"$d\_IP_Rep0.tagAlign\" --ipFile=$d\_IP_Rep0.tagAlign
		# macs2 peak calls
			...
			macs2 callpeak -t $d\_IP_Rep0.tagAlign.gz -c $d\_Input_Rep0.tagAlign.gz -g dm --to-large --nomodel --extsize \$ss -p 0.1 -n $d\_IP_Rep0.tagAlign_VS_$d\_Input_Rep0.tagAlign &
...
#			Rscript $Config{'bin_dir'}/run_spp.R -c=$d\_IP_Rep0.pr2.tagAlign.gz -i=$d\_Input_Rep0.tagAlign.gz -npeak=$npeaks -x=-500:85 -odir=./ -p=$node -filtchr='.*_[CD].*' -savr -savp -rf -out=$Config{'date'}.cc
		# macs2 peak calls
			...
			macs2 callpeak -t $d\_IP_Rep0.pr1.tagAlign.gz -c $d\_Input_Rep0.tagAlign.gz -g dm --to-large --nomodel --extsize \$ss -p 0.1 -n $d\_IP_Rep0.pr1.tagAlign_VS_$d\_Input_Rep0.tagAlign &
			ss=\$(cat $Config{'date'}.cc | grep $d\_IP_Rep0.pr2.tagAlign.gz | cut -f3 | head -n 1 | cut -d \",\" -f1)
			macs2 callpeak -t $d\_IP_Rep0.pr2.tagAlign.gz -c $d\_Input_Rep0.tagAlign.gz -g dm --to-large --nomodel --extsize \$ss -p 0.1 -n $d\_IP_Rep0.pr2.tagAlign_VS_$d\_Input_Rep0.tagAlign &'`
			wait \$!
	# create input wiggle file
#		Rscript $bin_dir/spp_wig_BAM.R  --args  --baseDir=./ --outDir=./ --factor=$d\_Input_Rep0 --ipFile=$d\_Input_Rep0.bam
	# sorting macs2 peaks
		ls -1 $d\*narrowPeak | xargs -P 8 -iFILES bash -c '
			OUT=`echo FILES | sed \'s\/_peaks.narrowPeak/.narrowPeak.gz\/\'`
			sort -k 8nr,8nr FILES | head -n 30000 | gzip -c > \$OUT
			if \[ -e \$OUT \]\;then rm \-f FILES\; fi\;
		\'\;
	#####################
	...
	if (scalar @IPs <= $node){
		...
		echo -n \$IPs | xargs -d\"\;\" -P 8 -iFILES bash -c '
			...
			FILE3=`echo FILES | sed \'s\/\.regionPeak\.gz\/\.narrowPeak\.gz\/'`
			FILE4=`echo FILES | sed \'s\/\.regionPeak\.gz\/\.narrowPeak\.gz\/' | sed \'s\/\.pr1\.tagAlign\/\.pr2\.tagAlign\/\'`
...
#			Rscript batch-consistency-plot.r 1 \$OUTPUT.pr1_VS_\$OUTPUT.pr2 \$OUTPUT.pr1_VS_\$OUTPUT.pr2
			Rscript batch-consistency-analysis.r \$FILE3 \$FILE4 -1 \$OUTPUT.pr1_VS_\$OUTPUT.pr2.macs 0 F p.value
		\'\;
";
...
";
			...
			print OUT2 "\t\t\tnumPeaks_Rep$R1\_pr_macs=\$(awk '\$11 <= $IDR_pr {print \$0}' $IPs[$i].pr1_VS_$IPs[$i].pr2.macs-overlapped-peaks.txt | wc -l)\n";
				...
				print OUT2 "\t\t\tif [ \$numPeaks_Rep$R2\_Rep$R1\_spp -gt \$max_numPeaks_Rep_spp ]; then\n\t\t\t\tmax_numPeaks_Rep_spp=\$numPeaks_Rep$R2\_Rep$R1\_spp\n\t\t\tfi\n";
				print OUT2 "\t\t\tif [ \$numPeaks_Rep$R2\_Rep$R1\_macs -gt \$max_numPeaks_Rep_macs ]; then\n\t\t\t\tmax_numPeaks_Rep_macs=\$numPeaks_Rep$R2\_Rep$R1\_macs\n\t\t\tfi\n";
                        }
                }
...
";
                print OUT2 "\t\t\tif [ \$numPeaks_Rep0_pr_spp -gt \$max_numPeaks_Rep_spp ]; then\n\t\t\t\toptThresh_spp=\$numPeaks_Rep0_pr_spp\n\t\t\telse\n\t\t\t\toptThresh_spp=\$max_numPeaks_Rep_spp\n\t\t\tfi\n";
                print OUT2 "\t\t\tif [ \$numPeaks_Rep0_pr_macs -gt \$max_numPeaks_Rep_macs2 ]; then\n\t\t\t\toptThresh_macs=\$numPeaks_Rep0_pr_macs\n\t\t\telse\n\t\t\t\toptThresh_macs=\$max_numPeaks_Rep_macs\n\t\t\tfi\n";
		print OUT2 "\t\tzcat $d\_IP_Rep0.tagAlign_VS_$d\_Input_Rep0.tagAlign.regionPeak.gz | sort -k7nr,7nr | head -n \$optThresh_spp | pigz -c > spp.optimal.$d\_IP_Rep0.tagAlign_VS_$d\_Input_Rep0.tagAlign.regionPeak.gz\n";
		print OUT2 "\t\tzcat $d\_IP_Rep0.tagAlign_VS_$d\_Input_Rep0.tagAlign.regionPeak.gz | sort -k7nr,7nr | head -n \$max_numPeaks_Rep_spp | pigz -c >  spp.conservative.$d\_IP_Rep0.tagAlign_VS_$d\_Input_Rep0.tagAlign.regionPeak.gz\n";
		print OUT2 "\t\tzcat $d\_IP_Rep0.tagAlign_VS_$d\_Input_Rep0.tagAlign.narrowPeak.gz | sort -k7nr,7nr | head -n \$optThresh_macs | pigz -c > macs.optimal.$d\_IP_Rep0.tagAlign_VS_$d\_Input_Rep0.tagAlign.narrowPeak.gz\n";
		print OUT2 "\t\tzcat $d\_IP_Rep0.tagAlign_VS_$d\_Input_Rep0.tagAlign.narrowPeak.gz | sort -k7nr,7nr | head -n \$max_numPeaks_Rep_macs | pigz -c >  macs.conservative.$d\_IP_Rep0.tagAlign_VS_$d\_Input_Rep0.tagAlign.narrowPeak.gz\n";
...
";
			print OUT3 "\t\tnumPeaks_Rep$R1\_pr_spp=\$(awk '\$11 <= $IDR_pr {print \$0}' $IPs[$i].pr1_VS_$IPs[$i].pr2-overlapped-peaks.txt | wc -l)\n";
			print OUT3 "\t\tnumPeaks_Rep$R1\_pr_macs=\$(awk '\$11 <= $IDR_pr {print \$0}' $IPs[$i].pr1_VS_$IPs[$i].pr2.macs-overlapped-peaks.txt | wc -l)\n";
			...
			for (my$j=$i+1;$j<@IPs;$j++){
				...
				print OUT3 "\t\tif [ \$numPeaks_Rep$R2\_Rep$R1\_spp -gt \$max_numPeaks_Rep_spp ]; then\n\t\t\tmax_numPeaks_Rep_spp=\$numPeaks_Rep$R2\_Rep$R1\_spp\n\t\tfi\n";
				print OUT3 "\t\tif [ \$numPeaks_Rep$R2\_Rep$R1\_macs -gt \$max_numPeaks_Rep_macs ]; then\n\t\t\tmax_numPeaks_Rep_macs=\$numPeaks_Rep$R2\_Rep$R1\_macs\n\t\tfi\n";
			}
		}
		print OUT3 "\t## IDR for combined Rep
";
		print OUT3 "\t\tnumPeaks_Rep0_pr_spp=\$(awk '\$11 <= $IDR_pr {print \$0}' $d\_IP_Rep0.pr1_VS_$d\_IP_Rep0.pr2-overlapped-peaks.txt | wc -l)\n";
		print OUT3 "\t\tnumPeaks_Rep0_pr_macs=\$(awk '\$11 <= $IDR_pr {print \$0}' $d\_IP_Rep0.pr1_VS_$d\_IP_Rep0.pr2.macs-overlapped-peaks.txt | wc -l)\n";
		print OUT3 "\t\techo \"numPeaks_Rep0_pr\t\$numPeaks_Rep0_pr_spp\t\$numPeaks_Rep0_pr_macs\"\n";
		print OUT3 "\t## print optimal and conservitive IDR values
";
		print OUT3 "\t\tif [ \$numPeaks_Rep0_pr_spp -gt \$max_numPeaks_Rep_spp ];\n\t\t\tthen optThresh_spp=\$numPeaks_Rep0_pr_spp;\n\t\t\telse opThresh_spp=\$max_numPeaks_Rep;\n\t\tfi\n";
		print OUT3 "\t\tif [ \$numPeaks_Rep0_pr_macs -gt \$max_numPeaks_Rep_macs ];\n\t\t\tthen optThresh_macs=\$numPeaks_Rep0_pr_macs;\n\t\t\telse opThresh_macs=\$max_numPeaks_Rep;\n\t\tfi\n";
		print OUT3 "\t\techo \"optThresh\t\$optThresh_spp\t\$optThresh_macs\"\n\t\techo \"conThresh\t\$max_numPeaks_Rep_spp\t\$max_numPeaks_Rep_macs\"\n";
	}
