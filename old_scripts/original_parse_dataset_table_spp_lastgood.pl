...
# Strain_Tissue_STAGE_Source_AbID_Replicate
	...
	{	print OUT1 "rsync -vcr avictorsen\@192\.170\.228\.37\:$prj_dir/$fn ./";
	}
	#if ($fn =~ /.gz$/) {
	#	print OUT1 "for file in ./$fn; do\ngzip -d \$file;\nperl ./idrpipeline/Phred64_to_Phred33.pl \${file%.gz} > \${file%.gz}.temp;\nmv \${file%.gz}.temp \${file%.gz} || { exit ;};\ndone\n";
...
for file in ./$fn; do
	case \$file in
		\*\.gz\)
		        check1=`zcat \$file | head -3000 | grep -c -m 20 "a"`;
		        check2=`zcat \$file | head -3000 | grep -c -m 20 "e"`;
		        if [ \"\$check1\" -gt 2 ] && [ \"\$check2\" -gt 2 ];then
                		echo "converting to Phred33";
		...
		\*\)
		        check1=`cat \$file | head -3000 | grep -c -m 20 "a"`;
		        check2=`cat \$file | head -3000 | grep -c -m 20 "e" \$file`;
		        if [ \"\$check1\" -gt 2 ] && [ \"\$check2\" -gt 2 ];then
                		echo "converting to Phred33";
...
###### convert Phred64 fastq's to Phred33's
#print OUT2 qq{
#for file in ./*.fastq; do
#	check1=`grep "a" $file`;
#	check2=`grep "\\^" $file`;
#	if [ \$\{\#check1\} -gt 2 ] && [ \${\#check2\} -gt 2 ];then
#		cat \$file | perl ./idrpipeline/Phred64_to_Phred33.pl > \$\{file%fastq\}.new;
#		mv \$\{file%fastq\}.new \$file;
#		echo "fastq quality scores converted to Phred33 format";
#	else
#		echo "fastq quality scores are already in Phred33 format";
#	fi;
	...
	foreach my $r (sort {$a <=> $b} keys %{$Dataset{$d}{'IP'}}) {
		...
		print OUT2 "## dataset $d IP replicate $r
			...
			$bwapath/bwa samse $Config{'reference'} $ip.sai $ip.$fastq_suffix > $ip.sam
                        if [ \$? -gt 0 ];then exit; else rm $ip.sai $ip.$fastq_suffix; fi;
                        $samtoolspath/samtools view -Sbh $ip.sam > $ip.bam
                        if [ \$? -gt 0 ];then exit; fi;
			$samtoolspath/samtools view -F 1548 -h -S -q 30 -o $ip\_q30.sam $ip.sam
			$samtoolspath/samtools view -Sb $ip\_q30.sam | $bedtoolspath/bamToBed -i stdin | awk 'BEGIN{FS=\"\\t\";OFS=\"\\t\"}{\$4=\"N\";print\$0}' | pigz -c > $ip.tagAlign.gz
			$Config{'bin_dir'}/shuf.sh $ip.tagAlign.gz ./ $ip
			$samtoolspath/samtools view -Sbh $ip\_q30.sam > $ip\_q30.bam
                        if [ \$? -gt 0 ];then exit; else rm $ip\_q30.sam; fi;
	...
	foreach my $r (sort {$a <=> $b} keys %{$Dataset{$d}{'Input'}}) {
		...
		print OUT2 "## dataset $d Input replicate $r
			...
			$bwapath/bwa samse $Config{'reference'} $input.sai $input.$fastq_suffix > $input.sam
                        if [ \$? -gt 0 ];then exit; else rm $input.sai $input.$fastq_suffix; fi;
			$samtoolspath/samtools view -Sbh $input.sam > $input.bam
			if [ \$? -gt 0 ];then exit; fi;
			cat $input.sam | awk 'BEGIN{OFS=\"\\t\"}{print \$1,\$2,\$3,\$6}' | grep -v $mito | sort | uniq -c | awk 'BEGIN{mt=0;m0=0;m1=0;m2=0} (\$1==1){m1=m1\+1} (\$1==2){m2=m2\+1} {m0=m0\+1} {mt=mt\+\$1} END{printf \"\%d\\t\%d\\t\%d\\t\%d\\t\%f\\t\%f\\t\%f\\n\",mt,m0,m1,m2,m0\/mt,m1\/m0,m1\/m2}' > $input.pbc.qc
                        $samtoolspath/samtools view -F 1548 -h -S -q 30 -o $input\_q30.sam $input.sam
			$samtoolspath/samtools view -Sb $input\_q30.sam | $bedtoolspath/bamToBed -i stdin | awk 'BEGIN{FS=\"\\t\";OFS=\"\\t\"}{\$4=\"N\";print\$0}' | pigz -c > $input.tagAlign.gz
			$samtoolspath/samtools view -Sbh $input\_q30.sam > $input\_q30.bam
                        if [ \$? -gt 0 ];then exit; else rm $input\_q30.sam; fi;
                        $bedtoolspath/bedtools bamtobed -i $input.bam | awk 'BEGIN{OFS=\"\\t\"}{print \$1,\$2,\$3,\$6}' | grep -v $mito | sort | uniq -c | awk 'BEGIN{mt=0;m0=0;m1=0;m2=0} (\$1==1){m1=m1\+1} (\$1==2){m2=m2\+1} {m0=m0\+1} {mt=mt\+\$1} END{printf \"\%d\\t\%d\\t\%d\\t\%d\\t\%f\\t\%f\\t\%f\\n\",mt,m0,m1,m2,m0\/mt,m1\/m0,m1\/m2}' > $input.pbc.qc
                        $bedtoolspath/bedtools bamtobed -i $input\_q30.bam | awk 'BEGIN{OFS=\"\\t\"}{print \$1,\$2,\$3,\$6}' | grep -v $mito | sort | uniq -c | awk 'BEGIN{mt=0;m0=0;m1=0;m2=0} (\$1==1){m1=m1\+1} (\$1==2){m2=m2\+1} {m0=m0\+1} {mt=mt\+\$1} END{printf \"\%d\\t\%d\\t\%d\\t\%d\\t\%f\\t\%f\\t\%f\\n\",mt,m0,m1,m2,m0\/mt,m1\/m0,m1\/m2}' > $input\_q30.pbc.qc
			";
		$all_inputs .= "$input.tagAlign.gz\t";
	}

	print OUT2 "## pool all IPs and inputs
		zcat $all_ips | pigz -c > $d\_IP_Rep0.tagAlign.gz
		zcat $all_inputs | pigz -c > $d\_Input_Rep0.tagAlign.gz
...
#		}
	...
	print OUT2 "# consistency: pooled-pseduoreplicates
		...
		zcat $d\_IP_Rep0.tagAlign_VS_$d\_Input_Rep0.tagAlign.regionPeak.gz | sort -k7nr,7nr | head -n \$optThresh | pigz -c > spp.optimal.$d\_IP_Rep0.tagAlign_VS_$d\_Input_Rep0.tagAlign.regionPeak.gz
		echo \"MSG: conThresh \$max_numPeaks_Rep\"
		zcat $d\_IP_Rep0.tagAlign_VS_$d\_Input_Rep0.tagAlign.regionPeak.gz | sort -k7nr,7nr | head -n \$max_numPeaks_Rep | pigz -c > spp.conservative.$d\_IP_Rep0.tagAlign_VS_$d\_Input_Rep0.tagAlign.regionPeak.gz
		\n";
...
print OUT2 "mail -s 'Run on '\$HOSTNAME' has finished!' -aFROM:\\<avictorsen\@IGSB.opensciencedatacloud.org\\> avictorsen\@gmail.com < $Config{'sp'}_idr.log";
