...
#for debugging, making warnings as errors
options(warn=2)
...
#	narrow.peaks <- find.binding.positions(signal.data=chip.data,control.data=control.data,fdr=iparams$fdr,method=tag.lwcc,whs=crosscorr$whs,cluster=cluster.nodes,tec.filter=F)
	if (!is.na(iparams$n.nodes)) {
		stopCluster(cluster.nodes)
	}
...
#		system(paste('gzip -f ',iparams$output.npeak.file))
		system(paste('pigz -f ',iparams$output.npeak.file))
	}
...
#		system(paste('gzip -f ',iparams$output.rpeak.file))
		system(paste('pigz -f ',iparams$output.rpeak.file))
	}
...
### call input subtracted wig
output.wig.file <- file.path(iparams$output.odir, paste(get.file.parts(iparams$chip.file)$name, '_VS_', get.file.parts(iparams$control.file)$name,'.wig', sep=""))
wigString <- paste (get.file.parts(iparams$chip.file)$name, ", smoothed, background-subtracted tag density", sep=" ")
cat(output.wig.file)
cat(wigString)
#smoothed.density <- get.smoothed.tag.density(chip.data,control.tags=control.data,bandwidth=200,step=100,tag.shift=tag.shift)
smoothed.density <- get.smoothed.tag.density(chip.data,control.tags=control.data,bandwidth=200,step=100)
writewig(smoothed.density, output.wig.file, wigString)
rm(smoothed.density)

#commented by Padma, using another script on R 2.15 to generate scaled wigs.
### call individual wig, normalized to 1M library size.
#output.chipwig.file <- file.path(iparams$output.odir, paste(get.file.parts(iparams$chip.file)$name,'.wig', sep=""))
#chipwigString <- paste (get.file.parts(iparams$chip.file)$name, ", smoothed and normalized", sep=" ")
#chipsmoothed.density <- get.smoothed.tag.density(chip.data,bandwidth=200,step=100,tag.shift=tag.shift,scale.by.dataset.size=T)
#chipsmoothed.density <- get.smoothed.tag.density(chip.data,bandwidth=200,scale.by.dataset.size=T)
#writewig(chipsmoothed.density, output.chipwig.file, chipwigString)
#rm(chipsmoothed.density)
